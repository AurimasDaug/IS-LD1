...
etot = abs(e(1)) + abs(e(2)) + abs(e(3)) + abs(e(4)) + abs(e(5));
...
while etot ~= 0 % executes while the total error is not 0
    % here should be your code of parameter update
%   calculate output for current example
    v = P(1, counter)*w1+P(2, counter)*w2+b;
    if v > 0
        y = 1;
    else
        y = -1;
...
%   update parameters using current inputs ant current error
    %fprintf('before  - %d \n', w1);
    w1 = w1+eta*e*P(1, counter);
    %fprintf('after - %d \n', w1);
    w2 = w2+eta*e*P(2, counter);
    b = b+eta*e;
...
for cycle = 1:5
    ...
    v = PM(1, cycle)*w1+PM(2, cycle)*w2+b;
